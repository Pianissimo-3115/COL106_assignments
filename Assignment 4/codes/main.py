...
def exhaustive_test_dynamic_hashset(dynamic_hashset):
    ...
    for i, key in enumerate(tqdm(keys, desc=f"Inserting keys ({dynamic_hashset.collision_type})", leave=False)):
        dynamic_hashset.insert(key)
...
def exhaustive_test_dynamic_hashmap(dynamic_hashmap):
    ...
    for i, (key, value) in enumerate(
            tqdm(zip(keys, values), desc=f"Inserting pairs ({dynamic_hashmap.collision_type})", leave=False)):
        dynamic_hashmap.insert((key, value))
