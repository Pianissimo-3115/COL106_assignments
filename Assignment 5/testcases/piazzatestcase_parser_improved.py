...
def main():
    ...
    with open(output_path, 'w') as output_file:
        ...
        for start_city, destination in tqdm(sampled_pairs, desc="Processing city pairs", unit="pair"):
            ...
            route1_start_time = time.time()
            route1 = flight_planner.least_flights_earliest_route(start_city, destination, 0, 3000000000000)
            route1_time = time.time() - route1_start_time
            write_to_output(output_file, f"Task 1: Took {route1_time}")
            if route1:
            ...
            route3_time = time.time() - route3_start_time
            write_to_output(output_file, f"Task 3: Took {route3_time}")
